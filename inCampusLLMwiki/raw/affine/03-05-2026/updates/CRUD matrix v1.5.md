...
| **Update Campus Insight Consent**       | Access and profile    | Allows the student to grant or revoke consent for identifiable campus insight access. The choice is stored on the student account and does not affect normal app access.                                              |          |          | RU       |                     |          |                     |                     |          |          |          |
| **View Consent-Based Student Insights** | Campus Administration | Allows an authorized campus admin to read identifiable student profile-interest and activity-participation insight data only when the student belongs to the admin’s authorized campus scope and has granted consent. | R        |          | R        | R (if consent=TRUE) |          | R (if consent=TRUE) | R (if consent=TRUE) |          |          |          |
...
1. **Pending Request Withdrawal**: If a user withdraws a *pending* join request, the system **mustn't** generate a notification for the host (aligned with team decision in D\&P v4), the notificaton is created only if the user has already joined the activity and then leave
