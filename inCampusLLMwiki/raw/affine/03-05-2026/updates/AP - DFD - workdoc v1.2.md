...
| 1.2     | 2026-05-01 |  Sections 3, 4.3, 5, 7.1, 8, 9, 11, 12, 13  | ed campus insight sharing as an AP-owned account/onboarding state stored in `DS-AP-001`                                                                                          | Student interest and activity-participation insight access requires explicit student consent and must not be treated as unrestricted admin access or ordinary public profile exposure. | Student insight access analysis; CRUD Matrix update           |
...
* student-controlled consent for campus insight sharing,
...
A new privacy-related boundary is also introduced: Access and Profile owns the student's consent state for campus insight sharing, because this consent belongs to the student account/onboarding domain. However, Access and Profile does not own the later admin insight view itself. Any future Campus Admin or campus-department view must consume the AP-owned consent state before reading identifiable student interests, profile information, or activity-participation history.
...
The third is **student consent state management**, which stores whether the student allows identifiable profile-interest and activity-participation insight data to be accessed by authorized campus staff for campus-life improvement purposes.

The fourth is **minimal profile lifecycle management**, which begins with the initial setup and continues with later editing.

The fifth is **restricted profile exposure**, meaning the controlled retrieval and display of minimal profile data only in allowed contexts and only if block rules allow access.
...
### 5.5 Consent-based campus insight sharing

The campus insight sharing decision is treated as part of the student account/onboarding state, not as ordinary public profile content. The student may explicitly grant or refuse permission for identifiable profile-interest and activity-participation insight data to be accessed by authorized campus staff for campus-life improvement purposes.

The consent value is stored in `DS-AP-001 Student Account` through the attribute `CampusInsightSharingConsent`.

If the value is `False`, identifiable student-interest and activity-participation insight data must not be exposed through future admin insight views. This refusal does not prevent normal app usage and does not remove safety or moderation access already required by report-review workflows.

This means that AP owns the consent state, while the future admin insight process must treat that state as an access-control condition before reading `DS-AP-002`, `DS-HL-001`, or `DS-HL-002`.
...
The subgroup needs this store to hold, at minimum, the student’s university email, verification status, activation state, platform access status, selected campus association according to the current CRUD operation row for **Select Campus**, and the student's consent state for campus insight sharing.Without this store, sign-up would have nowhere stable to create the verified account, sign-in would have nowhere to read it from, and campus selection would have nowhere to record the chosen campus on the student side.

The consent state is represented by `CampusInsightSharingConsent`. This attribute controls whether future authorized campus insight views may access identifiable student-interest and activity-participation insight data. It must not be confused with public profile visibility, because it governs institutional/admin insight access rather than ordinary student-to-student profile exposure.
...
| ----------------------------------------------------------- | ------------------ | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **DS-AP-001: Student Account / UserAccountStore**           | Access and Profile | Confirmed | Holds core account and access data for each student user, including university email, verification status, account activation/platform access state, selected campus association according to the current process-level CRUD row, and the student's consent state for campus insight sharing. It is created or updated during sign-up/verification, read during sign-in, updated during campus selection, and updated when the student grants or revokes campus insight sharing consent. |
...
| **Update Campus Insight Sharing Consent** | Allow the student to grant or revoke consent for identifiable campus insight access.   | `DS-AP-001`                               | `DS-AP-001`            | Reads the current account consent state and updates `CampusInsightSharingConsent`; refusal does not block normal app usage. |
...
The consent update process is logically part of Access and Profile because it changes account-level permission state. It does not expose student data by itself. It only determines whether a later authorized admin insight process may read identifiable student-interest and activity-participation data from AP and H\&L stores.
...
6. **Campus insight sharing consent must be modeled as an account-state update in&#x20;****`DS-AP-001`****.** The AP diagram may show this as an onboarding/settings branch, but it should not show the Campus Admin reading profile or participation data directly inside AP. AP only owns the consent state; future admin insight access is a separate read-only process that must first check this consent.
...
The exact user-interface placement of `CampusInsightSharingConsent` remains open. It may be collected during onboarding, exposed later in account settings, or both. What is fixed at this stage is the logical ownership: the consent state belongs to `DS-AP-001 Student Account`, and any future admin insight access must check it before exposing identifiable student-interest or activity-participation data.

The exact scope of future campus insight views also remains outside AP ownership. AP provides the consent state; it does not define the admin dashboard, aggregation logic, or campus-department reporting format.
...
For the future merged DFD, `CampusInsightSharingConsent` must be treated as an AP-owned permission state exported to any future consent-based campus insight process. The merged model should avoid drawing direct unrestricted access from Campus Admin to student profiles or participation history. Any such access must pass through a consent-checking process that reads `DS-AP-001` before conditionally reading `DS-AP-002`, `DS-HL-001`, or `DS-HL-002`.
