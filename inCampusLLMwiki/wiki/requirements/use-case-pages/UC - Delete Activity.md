...
Architecture note: H&L workdoc `v2.1` and CRUD Matrix `v1.5` resolve deletion as hard deletion of the activity record in `DS-HL-001` and all linked participation/request records in `DS-HL-002`. Deletion is distinct from cancellation and does not have a confirmed deletion-notification branch.
