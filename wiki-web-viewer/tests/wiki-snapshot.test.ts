...
async function loadWikiModule(
  ...
  process.env.HOME = setupConfigPath ? path.dirname(setupConfigPath) : root;
...
describe("wiki snapshot", () => {
  ...
  it("normalizes InCampus wiki-prefixed links for pages and graph edges", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "wiki-ui-"));
    ...
    try {
      ...
      await writeFile(
        ...
        "# Index\n\nRead [[wiki/project/overview|Project Overview]] next.\n",
      );
      await writeFile(
        path.join(root, "project", "overview.md"),
        "# Project Overview\n\nReturn to [[wiki/index|Wiki Index]].\n",
      );

      const wiki = await loadWikiModule(root);
      const index = await wiki.getWikiPage(["index"]);
      const graph = await wiki.getGraphData();

      expect(index.contentMarkdown).toContain(
  ...
  it("handles multi-word search with case and punctuation", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "wiki-ui-"));

    try {
      await writeFile(
        path.join(root, "Activity Notification.md"),
        "# Activity Notification\n\nActivity notification rules cover reminders and cancellation updates.\n",
      );
      await writeFile(
        path.join(root, "Join Request.md"),
        "# Join Request\n\nStudents send a join request before joining a hosted activity.\n",
      );
      await writeFile(
        path.join(root, "CRUD Matrix.md"),
        "# CRUD Matrix\n\nThe CRUD matrix records create, read, update, and delete responsibilities.\n",
      );
      await writeFile(
        path.join(root, "Campus Admin Report.md"),
        "# Campus Admin Report\n\nCampus admin report handling supports moderation review.\n",
      );
      await writeFile(
        path.join(root, "Block User.md"),
        "# Block User\n\nA student can block user interactions after a safety concern.\n",
      );

      const wiki = await loadWikiModule(root);

      expect((await wiki.searchWiki("activity notification"))[0]?.file).toBe(
        "Activity Notification.md",
      );
      expect((await wiki.searchWiki("join request"))[0]?.file).toBe("Join Request.md");
      expect((await wiki.searchWiki("CRUD matrix"))[0]?.file).toBe("CRUD Matrix.md");
      expect((await wiki.searchWiki("campus admin report"))[0]?.file).toBe(
        "Campus Admin Report.md",
      );
      expect((await wiki.searchWiki("notification cancellation"))[0]?.file).toBe(
        "Activity Notification.md",
      );
      expect((await wiki.searchWiki("block-user!!"))[0]?.file).toBe("Block User.md");
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
