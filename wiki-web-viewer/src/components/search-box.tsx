...
function SearchInput({
  ...
  return (
    <form onSubmit={onSubmit} className="group relative w-full">
      <div className="surface-raised relative flex items-center rounded-lg border border-[var(--border)] transition-colors duration-200 group-focus-within:border-[var(--teal)]">
        <Search className="pointer-events-none absolute left-5 top-1/2 h-5 w-5 -translate-y-1/2 text-[var(--muted-foreground)] transition-colors duration-200 group-focus-within:text-[var(--teal)]" />
        <input
          ref={inputRef}
          type="search"
          value={query}
          ...
          className="w-full rounded-lg bg-transparent py-4 pl-14 pr-16 text-base text-[var(--foreground)] outline-none placeholder:text-[var(--muted-foreground)]"
        />
        <button
          type="submit"
          className="absolute right-2 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-md bg-[var(--primary)] text-[var(--primary-foreground)] transition-[transform,background-color] duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] hover:bg-[var(--lavender)] active:scale-[0.94]"
        >
...
function splitBrandTitle(title: string) {
  ...
  const acronymMatch = title.match(/^(.+?)([A-Z][A-Z]+)$/);
  if (acronymMatch) {
    return { lead: acronymMatch[1], accent: acronymMatch[2] };
  }
    ...
    <div className="relative flex min-h-screen flex-col">
      <header className="relative flex items-center justify-between gap-2 px-4 pb-3 pt-[calc(env(safe-area-inset-top)+1.5rem)] sm:gap-3 sm:px-6 sm:pb-4 sm:pt-[calc(env(safe-area-inset-top)+1.25rem)]">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2.5">
          <button
            ...
            className="surface flex items-center gap-2 rounded-lg px-3 py-2 text-xs text-[var(--muted-foreground)] transition-[transform] duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] active:scale-[0.97] disabled:cursor-wait disabled:opacity-75 sm:px-3.5"
          >
            <RefreshCw
              className={`h-4.5 w-4.5 text-[var(--teal)] ${refreshBusy ? "animate-spin" : ""}`}
            />
            <span className="font-semibold tabular-nums text-[var(--foreground)]">
              {totalPages.toLocaleString()}
            </span>
            <span className="hidden sm:inline">{config.navigation.articlesLabel}</span>
          </button>
          <Link
            to="/graph"
      ...
      <main
        className={`relative flex flex-1 px-4 ${
          showResults ? "pt-2 sm:pt-4" : "pt-8 sm:pt-12"
        }`}
      >
        <div className={`mx-auto flex w-full max-w-7xl flex-col ${showResults ? "" : "animate-in"}`}>
          {!showResults && (
            <div className="grid gap-8 pb-8 lg:grid-cols-[minmax(0,1.25fr)_20rem] lg:items-end lg:pb-10">
              <div className="space-y-5">
                ...
                <div className="space-y-4">
                  <h1 className="font-display text-6xl leading-[0.92] text-[var(--foreground)] sm:text-7xl xl:text-[7rem]">
                    {brandTitle.lead}
                    <span className="text-[var(--primary)]">{brandTitle.accent}</span>
                  </h1>
                  <p className="max-w-3xl text-base leading-8 text-[var(--muted-foreground)] sm:text-[1.125rem]">
                    {config.tagline}
                  </p>
                </div>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
                <div className="surface flex items-start gap-3 rounded-lg px-4 py-4">
                  <FolderOpen className="mt-0.5 h-6 w-6 text-[var(--teal)]" />
                  <div>
                ...
                <div className="surface flex items-start gap-3 rounded-lg px-4 py-4">
                  <Network className="mt-0.5 h-6 w-6 text-[var(--lavender)]" />
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--muted-foreground)]">
                      Browse Modes
                    </p>
                    <p className="mt-2 text-sm leading-6 text-[var(--foreground)]">
                      Search, map, and project pulse from one workspace.
                    </p>
                  </div>
                </div>
                <div className="surface flex items-start gap-3 rounded-lg px-4 py-4">
                  <Activity className="mt-0.5 h-6 w-6 text-[var(--peach)]" />
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--muted-foreground)]">
                      Team Lens
                    </p>
                    <p className="mt-2 text-sm leading-6 text-[var(--foreground)]">
                      See the docs shaping the product against student isolation.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className={`w-full ${showResults ? "max-w-4xl" : "max-w-4xl"} ${showResults ? "" : "mb-8"}`}>
            <SearchInput
              query={query}
            ...
            {hasQuery && (
              <div className="animate-in surface-raised mt-3 overflow-hidden rounded-lg">
                {isSearching ? (
                  <div className="space-y-3 p-4">
                    {[1, 2, 3].map((item) => (
                      <div
                        key={item}
                        className="animate-pulse space-y-2 rounded-lg bg-[var(--secondary)] p-4"
                      >
                    ...
                    {results.map((result, index) => {
                      ...
                      return (
                        <Link
                          key={result.file}
                          to={`/wiki/${slug}`}
                          className={`animate-in group relative block px-5 py-4 transition-[background-color] duration-150 hover:bg-[var(--surface-highlight)] ${staggerClass}`}
                        >
                          <span
                            aria-hidden
