    ...
    <div className="flex min-h-screen flex-col">
      <header className="flex items-center justify-between gap-2 px-4 pb-3 pt-[calc(env(safe-area-inset-top)+1.5rem)] sm:gap-3 sm:px-6 sm:pb-4 sm:pt-[calc(env(safe-area-inset-top)+1.25rem)]">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2.5">
          <span className="surface flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs text-[var(--muted-foreground)] sm:gap-2 sm:px-3.5">
      ...
      >
        <div className="animate-in space-y-10 sm:space-y-12">
          <div>
            <span className="chip-peach inline-flex items-center gap-1.5 rounded-md px-3 py-1 text-[0.7rem] font-semibold uppercase tracking-[0.16em]">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
            {statCards.map((card, index) => (
              <div
                key={card.label}
                className={`surface-raised hover-lift stagger-${index + 1} animate-in relative overflow-hidden rounded-lg p-4 sm:p-5`}
                style={{ borderTop: `1px solid ${card.accent}` }}
              >
                ...
                return (
                  ...
                  >
                    <div className="flex min-w-0 items-center gap-2.5 sm:gap-3">
                      <span
                        className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md text-[0.65rem] font-semibold text-[var(--primary-foreground)]"
                    ...
                    <div className="flex shrink-0 items-center gap-2 sm:gap-3">
                      <div className="relative hidden h-1.5 w-20 overflow-hidden rounded-sm bg-[var(--secondary)] sm:block sm:w-32">
                        <div
