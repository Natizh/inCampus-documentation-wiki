...
import { getDefaultInCampusWikiRoot, getSampleVaultRoot } from "./app-paths";
...
type WikiRootSource = "env" | "saved" | "default" | "none";
...
function isAncestorDirectory(parentPath: string, childPath: string) {
...
export async function resolveWikiRuntimeSettings(): Promise<WikiRuntimeSettings> {
  ...
  const defaultWikiRootCandidate =
    process.env.NODE_ENV === "test" ? null : getDefaultInCampusWikiRoot();
  const defaultWikiRoot =
    defaultWikiRootCandidate && (await pathIsDirectory(defaultWikiRootCandidate))
      ? defaultWikiRootCandidate
      : null;
  ...
  const storedWikiRoot =
    storedConfigState.configError === null
      ...
      : null;
  const storedWikiRootIsDirectory = storedWikiRoot ? await pathIsDirectory(storedWikiRoot) : false;
  const storedWikiRootForSelection =
    defaultWikiRoot &&
    storedWikiRoot &&
    (!storedWikiRootIsDirectory || isAncestorDirectory(storedWikiRoot, defaultWikiRoot))
      ? null
      : storedWikiRoot;

  const selectedWikiRoot =
    forcedEnvWikiRoot ?? storedWikiRootForSelection ?? envWikiRoot ?? defaultWikiRoot;
  const wikiRootSource: WikiRootSource = forcedEnvWikiRoot
    ? "env"
    : storedWikiRootForSelection
      ? "saved"
      : envWikiRoot
      ? "env"
      : defaultWikiRoot
