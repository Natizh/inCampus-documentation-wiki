...
interface ScoredSearchCandidate {
...
function normalizeSearchPhrase(query: string) {
...
function loadSearchCandidatesByScan(db: SqliteDb): SearchCandidate[] {
  return db
    .prepare(`
      SELECT
        file,
        title,
        title_lower AS titleLower,
        content_lower AS contentLower,
        markdown
      FROM pages
...
function scoreSearchCandidate(
  ...
  if (phrase.length > 0) {
    if (candidate.titleLower.includes(phrase)) {
      score += 40;
    }

    const phraseCount = countTermOccurrences(candidate.contentLower, phrase);
    if (phraseCount > 0) {
      score += 8 + phraseCount * 3;
    }
  }

  for (const term of terms) {
    if (candidate.titleLower === term) {
      score += 30;
      matchedTerms.add(term);
    } else if (candidate.titleLower.includes(term)) {
      score += 14;
      matchedTerms.add(term);
    }
...
export async function searchWiki(
  ...
  try {
    ...
    candidates = loadSearchCandidatesByScan(db);
  }
  ...
  const scoredCandidates = candidates
    .map((candidate) => scoreSearchCandidate(candidate, terms, phrase))
    .filter((candidate) => candidate.score > 0);
  const allTermMatches = scoredCandidates.filter(
    (candidate) => candidate.matchedTerms.size === terms.length,
  );
  const candidatesToReturn = allTermMatches.length > 0 ? allTermMatches : scoredCandidates;
  ...
  return candidatesToReturn
    ...
    .slice(0, 20)
    .map<SearchResult>(({ candidate, score }) => ({
      file: candidate.file,
      score,
